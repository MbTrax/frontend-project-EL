...
const newBlock = ref({revert: false})
const model = defineModel()
const blockModalVisible = ref(false)
const addBlock = () => {
    newBlock.value = {revert: false}
...
const onCompleteNewBlock = () => {
    if (newBlock.value.index >= 0) {
        ...
        model.value.data.push({
            ...
            revert: newBlock.value.revert ?? false
        })
    }
...
<template>
    <div>
        ...
        <Dialog v-model:visible="blockModalVisible" modal header="Добавить новый блок" :style="{ width: '35rem' }">
            <form class="flex flex-column gap-4" @submit.prevent="onCompleteNewBlock">
                ...
                <div class="flex flex-column gap-2">
                    <label> Варианты ответа на английском </label>
