...
import CartLesson from "@/components/Cart/CartLesson.vue";
import {ref} from "vue";
import api from "@/api.js";

const data = ref([])
api.get('lessons', {type: 5}).then((res) => {
    data.value = res
})
</script>
...
<template>
    <div class="app-container">
        ...
        <div>
            <div class="grid">
                <CartLesson v-for="lesson in data" :data="lesson" @click="async () => {
                await router.push('/types/' + lesson.id)
            }" class="col-3"/>
            </div>
        </div>
    </div>
